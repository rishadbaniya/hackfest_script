// TODO : Make sure it works on windows as well
...
// TODO : Ask Sagar Dai or Aaadarsha Dai if the participant will only use github or what, if that is the case then i can make use of github developer API
...
let HACKATHON_START_DATE = new Date(2022, 7, 5, 12, 0, 0).getTime() / 1000;
...
const INPUT_CSV_DATA = fs.readFileSync(__dirname + "/output/github_url_extracted.csv", {encoding: 'utf8', flag: 'r'});
const delimiter = ',';
...
GLOBAL_JSON.forEach((data, i) => {
    if (data["Qualified Or Not"] === "Qualified" || data["Qualified Or Not"] === "Disqualified") {
    } else {
        GLOBAL_JSON[i]["Qualified Or Not"] = "Unresolved";
    }

    if (data['GithubURL'] === 'Not Submitted' || data[CSV_FIELDS.TEAM_NAME] === "N/A") {
...
try {childProcess.execSync("yes | rm -r AxBxCxDxExFxGxYx", {stdin: [0, "ignore", "ignore"], shell: "sh"})} catch (_) {}
    ...
    GLOBAL_JSON.forEach((data, index) => {
        if (data["Qualified Or Not"] == "Unresolved") {
            ...
            try {
                ...
                let command = `git clone ${data[CSV_FIELDS.GITHUB_URL]} AxBxCxDxExFxGxYx`;
                ...
                let commit_history = childProcess.execSync('git log --reverse --format="%at"', {encoding: "UTF-8", cwd: "./AxBxCxDxExFxGxYx", stdin: [0, 'ignore', 'ignore'], shell: "sh"});
                ...
                if (OLDEST_COMMIT_DATE <= HACKATHON_START_DATE || OLDEST_COMMIT_DATE >= HACKATHON_END_DATE) {
